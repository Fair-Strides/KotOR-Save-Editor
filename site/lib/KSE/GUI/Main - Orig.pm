...
#	$GUI{Adjuster}		= $GUI{MasterFrame}->Adjuster(-side=>'left')->packAfter($GUI{ControlsFrame}, -side=>'left');
	...
	$sizeh = 3;
	$sizew = 23;
	$GUI{ControlsList}	= $GUI{ControlsFrame}->Scrolled('HList', -scrollbars=>'oe', -height=>30, -font=>[-family=>'Helvitica', -size=>20], -columns=>1, -drawbranch=>0, -header=>0, -indicator=>0, -indent=>0, -itemtype=>'window', -selectborderwidth=>0, -separator=>'#', -width=>15)->pack(-fill=>'both', -pady=>5);

	$GUI{ControlsButton01} = $GUI{ControlsList}->Button(-text=>'General',		-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('General'); });
	$GUI{ControlsButton02} = $GUI{ControlsList}->Button(-text=>'Variables',		-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('Globals'); });
	$GUI{ControlsButton03} = $GUI{ControlsList}->Button(-text=>'Inventory',		-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('Inventory'); });
	$GUI{ControlsButton04} = $GUI{ControlsList}->Button(-text=>'Journal',		-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('Journal'); });
	$GUI{ControlsButton05} = $GUI{ControlsList}->Button(-text=>'Player',		-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('Player'); });
	$GUI{ControlsButton06} = $GUI{ControlsList}->Button(-text=>'NPCs',			-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('NPCs'); });
	$GUI{ControlsButton07} = $GUI{ControlsList}->Button(-text=>'Classes',		-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('Classes'); });
	$GUI{ControlsButton08} = $GUI{ControlsList}->Button(-text=>'Feats',			-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('Feats'); });
	$GUI{ControlsButton09} = $GUI{ControlsList}->Button(-text=>'Force Powers',	-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('Powers'); });
	$GUI{ControlsButton10} = $GUI{ControlsList}->Button(-text=>'Equipment',		-relief=>'flat', -activebackground=>$act_but_color2, -width=>$sizew, -height=>$sizeh, -font=>[-family=>'Helvitica', -size=>$sizef], -command=>sub { ShowPanel('Equipment'); });
	
	$GUI{ControlsList}->add('Control01', -widget=>$GUI{ControlsButton01});
	$GUI{ControlsList}->add('Control02', -widget=>$GUI{ControlsButton02});
	$GUI{ControlsList}->add('Control03', -widget=>$GUI{ControlsButton03});
	$GUI{ControlsList}->add('Control04', -widget=>$GUI{ControlsButton04});
	$GUI{ControlsList}->add('Control05', -widget=>$GUI{ControlsButton05});
	$GUI{ControlsList}->add('Control06', -widget=>$GUI{ControlsButton06});
	$GUI{ControlsList}->add('Control07', -widget=>$GUI{ControlsButton07});
	$GUI{ControlsList}->add('Control08', -widget=>$GUI{ControlsButton08});
	$GUI{ControlsList}->add('Control09', -widget=>$GUI{ControlsButton09});
	$GUI{ControlsList}->add('Control10', -widget=>$GUI{ControlsButton10});

	$def_but_color = $GUI{'ControlsButton01'}->cget('-background');
	print "def_but_color: $def_but_color\n";
	...
	$GUI{SavesList}		= $GUI{SavesFrame}->Scrolled('Listbox', -scrollbars=>'osoe', -selectmode=>'extended', -activestyle=>'none', -font=>[-family=>'Lucida Console Regular', -size=>15], -height=>26, -width=>20)->pack(-fill=>'both');
...
sub ToggleSavesFrame
{
	if($saves_shown == 1)
	{
		$GUI{SavesFrame}->pack(-side=>'left', -before=>$GUI{ControlsList}, -fill=>'both', -padx=>10, -pady=>5);
...
sub GameLoadPopup
{
	...
	Set_Icon($popupbox);
	$popupbox->Label(-wraplength=>250, -text=>"The save " . KSE::Functions::Saves::GetCurrentSave() . " is loading.\n\nPlease wait...")->pack(-fill=>'x');
...
sub GameSavePopup
{
	...
	$popupbox->geometry("250x100");
	Set_Icon($popupbox);
	$popupbox->Label(-wraplength=>250, -text=>"The save " . KSE::Functions::Saves::GetCurrentSave() . " is being saved and repacked.\n\nPlease wait...")->pack(-fill=>'x');
